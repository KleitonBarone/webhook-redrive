...
`TRACE_EXPORTER=none` disables export without breaking context propagation. Both the API and worker also accept `TRACE_EXPORTER=otlp`, with an explicit `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` such as `https://collector.example/v1/traces`. Use an existing trusted OTLP/HTTP collector; no backend or trace UI is added to Compose. The endpoint cannot contain userinfo, query parameters, or a fragment. Configure collector credentials through `OTEL_EXPORTER_OTLP_TRACES_HEADERS` (or the standard general headers variable), not the URL. Standard exporter TLS certificate settings are supported; require HTTPS outside an explicitly trusted local collector network.

The queue holds at most 2,048 spans, batches at most 256, and flushes every second. An export has a three-second deadline and no retry. The queue never blocks delivery when full. Export failures log only `trace export failed`, without collector responses, URLs, or headers. Exporter environment is administrator-controlled and is separate from webhook destination policy. Spans can be dropped; history in PostgreSQL is authoritative. Only `traceparent` is accepted and forwarded, never `baggage` or `tracestate`. Trace headers are diagnostic and are not part of the webhook signature.
...
| `webhook_workers_recent` | Worker IDs with a committed poll in the last minute |
| `webhook_worker_poll_age_seconds` | Age of the most recent committed poll, zero when none exists |
| `webhook_worker_completion_age_seconds` | Age of the most recent worker completion, zero when none exists |
| `webhook_database_bytes` | PostgreSQL database size, including other schemas; not host free space |

`ready` describes time eligibility on an unpaused endpoint, not whether concurrency or rate permits are available. `paused` counts pending attempts and expired leases on paused endpoints; live leases remain `in_progress`. Terminal cycle expiration counts as a `dead_letter` outcome, not an `expired` queue entry. Before a worker sweeps expiration, a past-deadline pending row can still appear in its pending queue category but cannot be dispatched. Gauges can move in either direction and reflect retained history. Migration 008 backfills durable cumulative counters and histograms; transactionally deferred accounting updates preserve them across restarts and retention. Restoring an older backup can reset totals. See [operational alerts and runbooks](operations.md#readiness-and-alerts).
